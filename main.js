...
var AIAgentSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    if (excalidrawAvailable) {
      ...
      new import_obsidian.Setting(containerEl).setName("Default diagram folder").setDesc("Folder for new diagrams created by the agent (leave empty for vault root).").addText((t) => {
...
function makeArrow(fromId, toId, rng, label) {
  const arrowId = newId();
  const elements = [];
  const arrow = {
...
var DiagramLayoutEngine = class {
  ...
  timeline(spec) {
    ...
    if (sorted.every((n2) => n2.timestamp)) sorted.sort((a, b) => a.timestamp < b.timestamp ? -1 : 1);
...
${node.timestamp}` : node.label;
  ...
  entityGraph(spec) {
    ...
    const REPULSION = 9e3, SK = 0.08, SL = 260, DAMP = 0.82;
    for (let iter = 0; iter < 50; iter++) {
      ...
      for (const e of spec.edges) {
        const a = pMap.get(e.from), b = pMap.get(e.to);
        if (!a || !b) continue;
...
`;
  ...
  async createDiagram(input) {
    ...
    const folder = input.folder ?? this.diagramDefaultFolder;
